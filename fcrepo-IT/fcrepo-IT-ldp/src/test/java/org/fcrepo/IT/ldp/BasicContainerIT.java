...
@Category(org.fcrepo.IT.ldp.IntegrationTestCategory.class)
public class BasicContainerIT extends org.fcrepo.IT.ldp.FcrepoLdpTestSuite {
    ...
    public static void setup() throws URISyntaxException, IOException {
        ...
        org.fcrepo.IT.ldp.FcrepoLdpTestSuite.baseUrl = UriBuilder.fromUri(
                org.fcrepo.IT.ldp.FcrepoLdpTestSuite.PROTOCOL + "://" + org.fcrepo.IT.ldp.FcrepoLdpTestSuite.HOSTNAME).port(


                org.fcrepo.IT.ldp.FcrepoLdpTestSuite.SERVER_PORT).path(
                org.fcrepo.IT.ldp.FcrepoLdpTestSuite.CONTEXT)
                .path(pid).build().toString();
    }

    @Before
    public void before() {
        org.fcrepo.IT.ldp.FcrepoLdpTestSuite.log.debug("Performing required LDP re-initialization...");
        final String container = RestAssured
                .given()
                .header(HttpHeaders.CONTENT_TYPE, RDFFormat.TURTLE.getDefaultMIMEType())
                .header(HttpHeaders.LINK, Link.fromUri(LdpService.InteractionModel.LDPC
                        .stringValue()).rel(LdpWebService.LINK_REL_TYPE).build().toString())
                ...
                .put(org.fcrepo.IT.ldp.FcrepoLdpTestSuite.baseUrl)
        ...
        final String resource = RestAssured
                ...
                .statusCode(HttpStatusSuccessMatcher.isSuccessful())
                .header(HttpHeaders.LOCATION, CoreMatchers.notNullValue())
                .post(org.fcrepo.IT.ldp.FcrepoLdpTestSuite.baseUrl)
                .getHeader(HttpHeaders.LOCATION);

        RestAssured.reset();
        org.fcrepo.IT.ldp.FcrepoLdpTestSuite.log.info("Container: {}", container);
    ...
    public void testRunSuite() {
        ...
        Assert.assertTrue("ldp-testsuite is empty - no test run", (testSuite.getStatus() &
                org.fcrepo.IT.ldp.FcrepoLdpTestSuite.TESTNG_STATUS_HAS_NO_TEST) == 0);
        if ((testSuite.getStatus() & org.fcrepo.IT.ldp.FcrepoLdpTestSuite.TESTNG_STATUS_HAS_SKIPPED) != 0) {
            org.fcrepo.IT.ldp.FcrepoLdpTestSuite.log.warn("ldp-testsuite has skipped some tests");
        }
    }
